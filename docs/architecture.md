...
- **Composition planner**: `CompositionPlanner` expands prompts into deterministic multi-section `CompositionPlan` structures (tempo, key, sections, transitions). Plans are generated server-side if the CLI does not provide one and are embedded in every artifact’s metadata for reproducibility. The planner now assumes a long-form arc (intro → motif → chorus → outro, with an optional bridge for longer jobs), allocates bar counts to keep sections under the MusicGen token ceiling, and emits per-section orchestration layers (rhythm, bass, harmony, lead, textures, vocals) so downstream services and the CLI can reason about instrumentation. Requests below ~90 s still fall back to compact templates so legacy short-form workflows and automated tests remain fast.
- **Motif conditioning**: Each section now carries a `motif_directive`, variation axes, and cadence hints so backends know when to restate, evolve, or dissolve the theme. The CLI mirrors these fields for preview and reproducibility.
- **Inference backends**: a `ComposerOrchestrator` selects between lazy-loaded backends (Riffusion, MusicGen) per section, requests audio renders, and stitches them together with loudness normalisation. Crossfades are now avoided by default; sections butt together with short, click-safe envelopes and only fall back to longer overlaps when conditioning gaps or placeholders appear. Each backend reports section-level extras (seed, guidance, orchestration notes, placeholder status) that flow back to the CLI.
- **Guided motif reuse**: The orchestrator captures the first fully stated motif as an audio seed, analyses its chroma profile, and persists the clip alongside the final mix. MusicGen consumes this seed (plus the previous section tail) as an audio prompt to drive continuity; crossfade lengths adapt based on whether adjacent sections were conditioned or placeholders.
